   ...
   window.xjb_library = {
      intro: {
         "mingxie": {
            headline: "鸣谢",
            writer: "",
            content: `鸣谢B龇B牙B哥对本扩展的推荐。 n
            鸣谢B文B和B先B生对审批群的帮助。 n
            鸣谢B没B有B缓B冲提供的称号。 n
            鸣谢B阿B巴B～B阿B巴对扩展问题的热心帮助。 n
            鸣谢一众B杀B友的bug反馈。 n`,
            style: siyan
         },
         "qzj": {
            ...
            writer: "",
            content:
               `B强B制B技(qzj):一种特殊的主动技。n 
             B特B性:使技能目标该回合所有技能数据清空。n
             B提B示:请不要于此时用游戏自带编辑器编辑扩展，防止数据丢失！`,
            style: siyan
         },
         "junchenSkill": {
            headline: "君臣技",
            ...
            content: `
            B君B主B技(juくnSkill)n
            特性:非身份场视为一般技能；身份场为主公技。n
            B名B臣B技(cheくnSkill)n
            特性:成为主公技的目标后，待主公技结算后发动名臣技加粗部分。
            `,
            style: siyan
         },
         "hun_system": {
            ...
            writer: "",
            content: `
            B魂B币B系B统:一种交换系统，有如下功能：n
            A商A店n
            用于交易特殊卡牌，现对金币、铜币添加了防伪标记。n
            A变A身n
            使用后变成对应的武将(实际上是获得一个随从)，自带装备和卡牌。当一名变身武将死亡后才可变身为另外的武将。n
            A养A成n
            点击养成功能的各选项即开始养成。n
            A维A持A魂A币A系A统：n
            在使用上述系统功能以及某些魂币系统提供时，会消耗系统能量。n
            消费魂币会让系统获得能量。n
            ...
            B两B种B货B币：场上角色累积使用牌、打出牌、造成伤害的分数达到500，牌堆中便添加一张【铜币】；
         ...
         "skill_X": {
            ...
            writer: "",
            content: `
            AXA技(skill_X):为方便独立出来的技能。每一个数字对应一个效果。一般和player.fc_X一起使用。n
            ApAlAaAyAeAr.BfBcB_BX:X技的数据处理器。用法如下:n
             1.B数B字B类B参B数，一个数字只对应着一项事件(摸牌、恢复体力、失去体力等等)，可以连写，会依次执行。n
             2.B布B尔B类B参B数，布尔用于确认是否选择角色。true→令角色执行事件，false→oくnlyme角色执行事件。 n
             3.B数B组B类B参B数，数组的每一项和数字对应，并决定着事件的数值。n            
             4.B字B符B串B类B参B数，当这类参数为如下所提到的，会有如下效果:n
             ①くnoskill_temp:强制技效果；n
             ②baibaくn_temp:执行角色该回合白板；n
             ③usecheくnSkill:使用名臣技；n
             ④S:S之；n
             ⑤deS:取消S之。n
             5.B对B象B类B参B数，其键有如下所提到的，会有如下效果:n
            ①くnature:数组，为伤害设置属性；n
            ②expire:对象，为tempSkill设置期限;n
            ③skills:数组，设置获得的技能;n
            ④ideくntity:数组，设置成为的身份；n
            ⑤awakeくn:数组，设置废除的技能；n
            ⑥oくnlyme:数组，设置oくnlyme角色。n   
            n
            B提B示:每一个step中每一个player只能调用一次该函数，否则会出错。n            
            `,
            style: siyan
         },
      },
...
　孤闻介推之避晋封，申胥之逃楚赏，未尝不舍书而叹，有以自省也。奉国威灵，仗の钺征伐，推弱以克强，处小而禽大。意之所图，动无违事，心之所虑，何向不济，遂荡平天下，不辱主命。可谓天助汉室，非人力也。然封兼四县，食户三万，何德堪之！江湖未静，不可让位；至于邑土，可得而辞。今上还阳夏、の柘、苦三县户二万，但食武平万户，且以分损谤议，っ4shāo少减孤之责也。
`,
            style: wuyan
         },
         "隆中对": {
...
`,
      ...
      lingli: {
         "灵力简论": {
            ...
            content: `
   A灵A的A基A本A属A性A n
   1.灵的自然性:我们所定义的和灵有关的量，其绝对值必为自然数倍的最小单位量或者零，其他的解是无意义的解。n
   2.灵的进化性:目前关于灵的一切理论都是暂时的理论，灵力会自我“进化”。n
   nn
   A灵A魔A关A系 n
   1.规定:灵力或者魔力的最小单位量为1Ch。用字母L表示灵力。n
   2.规定：灵理论中，能量的最小单位量为1C。n
   1C等于销毁一张基本牌的能量变化。-1C表示生成一张基本牌的能量变化n
   3.规定:减少一点体力/体力上限能量变化为1H。相对的，增加一点体力/一点体力能量变化为-1H。n
   且1H=2C n
   4.规定:n
   若有1Ch，其所带能量为1C/0，则其为1Ch魔力。n
   若有1Ch，其所带能量为-1C，则其为1Ch灵力。n
   则1Ch魔力(1C)→1Ch灵力(-1C)，ΔE=-2C=-1H n
   则1Ch灵力(-1C)→1Ch魔力(0)，ΔE=C n
   注:ΔE表示能量变化，下同。 n
   5.灵魔转化的情况: n
   情况1(1Ch魔力→1Ch灵力): n
   转化关系:-ΔE=H=2C n     
   情况2(1Ch灵力→1Ch魔力): n
   转化关系:-ΔE=-1C n
   以上两个转化关系只表示初末能量变化，在灵力变为魔力的过程中，则是无法确定的，不过一般变化过程小于3个步骤。  n
   nn
   A灵A力A场 n
   1.灵力来源于灵力场。灵力场被一种叫灵颤的物质划分为9层，层的符号是S。n
   九层依次记为:n
   ...
   V(x)-V(x-1)=6(x-1)n
   将以上各式相加：n
   V(x)-V(1)=6[1+2+3+...+(x-1)] n
   V(x)-1=6[1+(x-1)](x-1)/2 n
   V(x)=3x(x-1)+1 n
   即V(x)=3x(x-1)+1 n
   当x=1时，3×1×(1-1)+1=1 n
   故:n
   ...
   4.规定:灵力密度为D，D=L/V。n
   D=0.47746485261×10^-3Ch/m³
   或 n
   D=2Ch/V。n
   灵力场中，灵力密度处处相等。n
   5.灵颤：在两个S间，有一个物质在两个S间来回运动，我们把这种物质叫做颤或灵颤，记为W，记8个颤分别为：n
   ...
   7.颤的频率的最小单位量为1Hz。 n
   8.灵动:n 
   灵颤的频率和灵颤左右层的灵力差之积为一个定值。即ΔLf=5040Ch·Hz(5040Ch/s)。n
   规定：该定值称为灵力的迁移速度(率)，简称灵动，符号为M。
   ...
   如果W1左侧那一层的灵力为0Ch，灵力就会呈现像我们4中所说的那种关系。n
   11.ΔLf=M推广：n
   灵力分界面上：n
   由ΔLf=M知，n
   (L外-L9)f=(M+M外)/2，n
   ...
   后同W7n
   15.灵力场体积变化表n
   从外到内依次减小，直到有一层达到体积最小，之后重复。n
   7V 19V 37V 61V 91V 127V 169V n
   7V 19V 37V 61V 91V 127V 128V n
   7V 19V 37V 61V 91V 092V 128V n   
   7V 19V 37V 61V 62V 092V 128V n 
   7V 19V 37V 38V 62V 092V 128V n   
   7V 19V 20V 38V 62V 092V 128V n 
   7V 08V 20V 38V 62V 092V 128V n  
   2V 08V 20V 38V 62V 092V 128V n  
   
   2V 08V 20V 38V 62V 092V 128V n   
   2V 08V 20V 38V 62V 092V 093V n   
   2V 08V 20V 38V 62V 063V 093V n 
   2V 08V 20V 38V 39V 063V 093V n   
   2V 08V 20V 21V 39V 063V 093V n   
   2V 08V 09V 21V 39V 063V 093V n  
   2V 03V 09V 21V 39V 063V 093V n    
   
   2V 03V 09V 21V 39V 063V 064V n   
   2V 03V 09V 21V 39V 040V 064V n
   2V 03V 09V 21V 22V 040V 064V n 
   2V 03V 09V 10V 22V 040V 064V n
   2V 03V 04V 10V 22V 040V 064V n
   
   2V 03V 04V 10V 22V 040V 041V n
   2V 03V 04V 10V 22V 023V 041V n  
   2V 03V 04V 10V 11V 023V 041V n    
   2V 03V 04V 05V 11V 023V 041V n
   
   2V 03V 04V 05V 11V 023V 024V n   
   2V 03V 04V 05V 11V 012V 024V n
   2V 03V 04V 05V 06V 012V 024V n 
   
   2V 03V 04V 05V 06V 012V 013V n
   2V 03V 04V 05V 06V 007V 013V n   
   
   2V 03V 04V 05V 06V 007V 008V n 
   nn
   A灵A力A转A化A原A理n
   1.反灵颤子和反灵颤子器n   
   反灵颤子器可以减小灵力场的层的体积，根据灵力密度不变，可以推知灵力变化，从而推知灵颤的频率变化。n
   2.阵法是天然的反灵颤子器，通过向其中加卡牌可以启动。卡牌振动频率与阵法振动频率相同时，便立即将卡牌吸引进去。n
   3.灵力饱和原理： n
   若L>0,不等式：n
